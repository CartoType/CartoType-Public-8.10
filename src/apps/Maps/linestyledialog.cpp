...
LineStyleDialog::LineStyleDialog(QWidget& aParent,CStyleItem& aStyleItem,CartoType::Legend& aLegend):
    QDialog(&aParent),
...
void LineStyleDialog::DrawLineSample(QLabel& aText,QLabel& aImage,int aScale)
    ...
    m_legend.Clear();
    m_legend.AddMapObjectLine(CartoType::MapObjectType::Line,"_temp",CartoType::FeatureInfo(),"","");
    CartoType::Result error;
    auto bitmap = m_legend.CreateLegend(error,2,"cm",{},aScale);
    if (bitmap)
        {
