...
void MapForm::CreateLegend()
    ...
    auto legend = std::make_unique<CartoType::Legend>(*m_framework,style);
    m_framework->SetLegend(std::move(legend),1.5,"in",CartoType::NoticePosition::TopRight);
    }
...
void MapForm::CreateTurnInstructions()
    ...
    m_framework->SetTurnInstructions(false,30,"mm",turn_instruction_position);
    }
        ...
        if (r)
            ...
            if (r->RouteSegment.size())
                ...
                if (s->Path.Points())
                    {
                    RoutePoint p;
                    p.Point.Point = s->Path.Point(s->Path.Points() - 1);
                    m_route_point_array.push_back(p);
                    }
                }
            }
...
void MapForm::Print(bool aPreview)
    ...
    auto CreateImage = [&](QRectF aPageRect)->QImage
        ...
        CartoType::Result error;
        auto legend_bitmap = legend.CreateLegend(error,2,"in",{});
        if (!error)
            {
