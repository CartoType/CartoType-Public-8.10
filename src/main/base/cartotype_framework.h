...
Copyright (C) 2012-2024 CartoType Ltd.
...
/**
FrameworkEngine holds the data accessors, fonts, and any parameters specific to data providers.
...
class FrameworkEngine
    ...
    /** Returns the parameters specific to data providers. For internal use only. */
    std::shared_ptr<std::map<std::string,std::string>> ProviderParam() const { return iProviderParam; }
    ...
    std::shared_ptr<std::map<std::string,std::string>> iProviderParam = std::make_shared<std::map<std::string,std::string>>();
...
class FrameworkMapDataSet
    ...
    Result InsertPointMapObject(uint32_t aMapHandle,const String& aLayerName,Point aPoint,
                                const String& aStringAttributes,FeatureInfo aFeatureInfo,uint64_t& aId,bool aReplace);
    Result InsertEllipseMapObject(uint32_t aMapHandle,const String& aLayerName,PointFP aCenter,double aRadiusX,double aRadiusY,double aRotationDegrees,
                                  const String& aStringAttributes,FeatureInfo aFeatureInfo,uint64_t& aId,bool aReplace);
...
class Framework: public MNavigatorObserver
    ...
    */
    ...
    void SetScaleBar(double aWidth,const char* aUnit,const ExtendedNoticePosition& aPosition);
    void EnableScaleBar(bool aEnable);
    bool ScaleBarEnabled() const;
    Result SetTurnInstructions(bool aAbbreviate,double aWidth,const char* aUnit,const ExtendedNoticePosition& aPosition,double aTextSize = 7,const char* aTextSizeUnit = "pt");
    ...
    uint32_t BuiltInProfileCount();
    const RouteProfile* BuiltInProfile(uint32_t aIndex);
    Result SetBuiltInProfile(uint32_t aIndex);
    ...
    std::unique_ptr<CartoTypeCore::Route> CreateBestRoute(Result& aError,const RouteProfile& aProfile,const RouteCoordSet& aCoordSet,bool aStartFixed,bool aEndFixed,uint32_t aIterations);
    std::unique_ptr<CartoTypeCore::Route> CreateBestRoute(Result& aError,const RouteProfile& aProfile,const CoordSet& aCoordSet,CoordType aCoordType,bool aStartFixed,bool aEndFixed,uint32_t aIterations);
    ...
    Result CreateRouteAsync(const RouterAsyncCallBack& aCallBack,const RouteProfile& aProfile,const RouteCoordSet& aCoordSet,bool aOverride);
    Result CreateBestRouteAsync(const RouterAsyncCallBack& aCallBack,const RouteProfile& aProfile,const RouteCoordSet& aCoordSet,bool aStartFixed,bool aEndFixed,uint32_t aIterations,bool aOverride);
    ...
    Result Construct(const Param& aParam,const CMap* aMapToCopy = nullptr);
