...
class RouteSegment
    ...
    */
    ...
    /** Turn instructions provided by the routing system. If not empty they are used instead of instructions generated from the information in this RouteSegment object. */
    String Instructions;
    };
...
class NearestRoadInfo
    ...
    /** The road's geometry in map coordinates. The road is guaranteed to be traversible in the direction of Path. */
    Contour Path;
    /**
...
class NavigatorTurn: public Turn
    ...
    Distance and/or time are used if positive, otherwise are taken from the previous segment if non-null.
    */
    NavigatorTurn(const RouteSegment* aPrevSegment,const RouteSegment& aCurSegment,double aDistance = -1,double aTime = -1);
