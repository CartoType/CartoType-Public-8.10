/*
cartotype_base.h
Copyright (C) 2004-2024 CartoType Ltd.
...
class Point
    ...
    explicit Point(InputStream& aInput);
...
class Rect
    ...
    explicit Rect(InputStream& aInput): Min(aInput), Max(aInput) { }
    /** Serializes a Rect to an output stream. */
    void Write(OutputStream& aOutput) const { Min.Write(aOutput); Max.Write(aOutput); }
    ...
    */
    ...
    Rect Expanded(int32_t aExpansion) const noexcept
        {
        return Rect(Min.X - aExpansion,Min.Y - aExpansion,Max.X + aExpansion,Max.Y + aExpansion);
        }
...
class RectFP
    ...
    explicit RectFP(InputStream& aInput);
    void Write(OutputStream& aOutput) const;
    /** The equality operator. */
    ...
    */
    ...
    RectFP Expanded(double aExpansion) const noexcept
        {
        return RectFP(Min.X - aExpansion,Min.Y - aExpansion,Max.X + aExpansion,Max.Y + aExpansion);
        }
...
inline double GreatCircleDistanceInMeters(double aLong1,double aLat1,double aLong2,double aLat2)
    ...
    meaning 'indefinite', which becomes INT32_MIN when converted to an integer. Observed on 64-bit Windows.
...
inline double AngleInDegreesNormalized(double aAngle)
    {
...
This value, 3, allows fast text searching at the cost of a few tens of megabytes of RAM.
Text searching is much slower if fewer than 3 levels are loaded.
*/
...
enum class MapObjectPurpose
    {
    /** Map objects are loaded for drawing the entire map. */
    General,
    /** Map objects are loaded for drawing a vector tile. */
    Tile,
    /** Map objects are loaded for drawing labels; objects with no string attributes and no icon need not be loaded. */
    Labels,
    /** Map objects are loaded as part of a search. */
    Search
    };
