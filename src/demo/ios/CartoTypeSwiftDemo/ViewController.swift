...
class ViewController: CartoTypeViewController, CartoTypeCreateRouteAsyncProtocol, UISearchBarDelegate, CLLocationManagerDelegate
{
    ...
    func createTurnInstructions()
        {
        let turn_instr_pos = CartoTypeExtendedNoticePosition(NoticePositionTopLeft, xInset: 12 * m_ui_scale, xUnit: "px", yInset: (Double(m_insets.top) + 12) * m_ui_scale, yUnit: "px")
        m_framework.setTurnInstructions(true, width: Double(view.bounds.size.width - 54) * m_ui_scale, widthUnit: "px", extendedPosition: turn_instr_pos, textSize: 12, textSizeUnit: "pt")
        }
        
    func createScaleBarAndTurnInstructions()
        {
        let scale_bar_pos = CartoTypeExtendedNoticePosition(NoticePositionBottomLeft, xInset: 12 * m_ui_scale, xUnit: "px", yInset: (Double(m_insets.bottom) + 34) * m_ui_scale, yUnit: "px")
        m_framework.setScaleBar(Double(view.bounds.size.width - 24) * m_ui_scale,widthUnit: "px",extendedPosition: scale_bar_pos)
    ...
    func startShowingLocation()
        ...
        if (m_framework.routeCount == 0)
            {
            m_framework.followMode = FollowModeLocation
    ...
    func startNavigating()
        ...
        if (m_framework.routeCount != 0)
            ...
            alert.addAction(UIAlertAction(title: "OK", style: .default, handler:
                ...
                self.m_framework.followMode = FollowModeLocationHeadingZoom
